...
The companion SCIM source changes start at Better Auth tag `v1.7.5`, commit `5468e6bfcdff799848537cf5ad06ebab15aad9dd`. They add optional `groups.maxMembers`, `bulk`, and `bulk.jobs` configuration, complete internal membership and identity pagination, chunked user lookups, and per-resource Bulk dispatch. The SCIM suite passes 368 tests and the full Better Auth workspace typecheck passes. A combined adapter/plugin contract passes with 1,051 Bulk-created users and a 1,051-member group through create, complete reads, replacement, PATCH, deletion, reprovisioning, and connection decommissioning. Retirement reconciles all 1,051 users, removes their projected grants, and rejects retired credentials. This contract passes with both the independent DynamoDB command double and [DynamoDB Local in CI](https://github.com/tgundhus/betterauth-dynamodb/actions/runs/35736107738). Neither is an AWS performance benchmark. The companion package is not published; installing `@better-auth/scim@1.7.5` does not enable these features.

The companion changes are available in [Better Auth continuation PR #1](https://github.com/tgundhus/better-auth/pull/1). Adapter CI pins revision `5f33f4cb1b08040bd90758b5bb0de9cc57e896f0`, builds that SCIM package, runs its tests, and exercises the combined scale and asynchronous-job contracts against DynamoDB Local. The pin includes transaction-scoped membership batches, bounded projection work, and user-indexed grant removal. See the CI result before treating a new revision as validated.
...
The fork supplies authenticated status polling and `runSCIMBulkWorker`, which uses a durable queue cursor, per-job leases, revision fences, original scope checks, and current credential validation. Cleanup removes payloads and results in bounded transactions while retaining an idempotency record. The native contract passes with 55 users, a dependent group, large Unicode payloads, fresh auth instances, result pagination, and cleanup. A separate native race test confirms that concurrent user deactivation prevents an already staged group grant from committing. See the [complete job and Lambda guide](https://github.com/tgundhus/better-auth/blob/5f33f4cb1b08040bd90758b5bb0de9cc57e896f0/docs/content/docs/plugins/scim/bulk-jobs.mdx).
...
1. Validate the deployed Lambda and gateway integration, current credential resolution, scheduled job and recovery workers, retention, alarms, and individual resource deadlines.
2. Run sustained concurrent provisioning and sign-in with production projection callbacks. Native regression tests cover group mutation racing deactivation and SSO sign-in racing deactivation; these do not establish production contention behavior. Query predicates do not receive SQL-style range locks.
