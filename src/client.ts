...
export function normalizeOptions(options: BetterAuthDynamoDBOptions): Required<Pick<BetterAuthDynamoDBOptions, "maxPages" | "unsafeAllowScan">> & BetterAuthDynamoDBOptions {
  ...
  return { ...options, ...(uniqueFields ? { uniqueFields } : {}), ...(pageSize ? { pageSize } : {}), maxPages: positiveSafeIntegerOption("maxPages", options.maxPages, 25), unsafeAllowScan: options.unsafeAllowScan ?? false };
}
