...
- `maxBulkConcurrency?: number` - positive safe integer concurrency cap for independent `updateMany`/`deleteMany` transactions, scalar-field `IN` queries, chunked `BatchGet` reads, and companion SCIM projection work for distinct users inside a callback transaction; defaults to `8`. Outside a callback transaction, a mutation failure can follow successful mutations, so bulk writes are not aggregate-atomic and must not be blindly retried.
...
Callback transactions have no fixed total item-count cap. The adapter handles native request limits internally. Transaction-scoped multi-row creates batch their strong absence checks in groups of at most 100 keys, then stage every row under the same atomic commit decision. The companion SCIM fork also uses `maxBulkConcurrency` for independent projection work on distinct users. This adds storage and request costs and requires an application-owned recovery worker. Individual DynamoDB item limits, deployment deadlines, and advertised SCIM HTTP limits still apply. A Bulk envelope commits each resource operation independently, as required by its protocol.
