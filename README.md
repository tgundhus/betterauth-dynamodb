...
This branch contains the **2.0 transaction preview**. It adds an opt-in storage protocol for callback transactions; large-group SCIM and `/Bulk` implementation and validation are still in progress. It is not a production compatibility claim. See [transaction storage and migration](./docs/transaction-storage.md). The stable 1.2 release keeps its existing storage contract and does not support callback transactions.
...
- `maxPages?: number` - positive safe integer maximum DynamoDB Query pages drained for sidecar equality lookups and explicit model-partition fallbacks; defaults to `25`. The adapter throws at this cap if more pages are needed to complete the result. Unsorted limited equality/model reads can finish earlier once enough live, matching rows satisfy offset and limit.
...
With Better Auth's `@better-auth/scim` and `@better-auth/sso` packages at `1.7.5`, the stable 1.2 adapter has these limitations. The 2.0 preview's callback transaction implementation is undergoing integration and plugin validation; the table below is not a compatibility claim for that preview.
